...
# ==== 构造训练数据 ====
# 扩展 Cp1_residual 和 Cp2_residual 为二维数组
Cp1_residual_2d = Cp1_residual.repeat(10, axis=0).reshape(-1, 1)  # 将 Cp1_residual 转换为二维数组
Cp2_residual_2d = Cp2_residual.repeat(10, axis=0).reshape(-1, 1)  # 将 Cp2_residual 转换为二维数组
...
X = np.hstack([
    ...
    Cp1_residual_2d,  # 添加 Cp1残差
    Cp2_residual_2d   # 添加 Cp2残差
])
y = Hvap.flatten()
...
def residuals(params, X, y):
    Nk = X[:, :19]  # 19个基团列
    MW = X[:, 19].reshape(-1, 1)  # 分子量
    Nc = X[:, 20].reshape(-1, 1)  # 碳原子数
    T = np.clip(X[:, 21].reshape(-1, 1), 1e-6, None)  # 温度
    slope = X[:, 22].reshape(-1, 1)  # slope × T 特征
    ...
    # 额外的系数（残差项）
    residual_Cp1_factor = params[121]  # Cp1残差的待定系数
    residual_Cp2_factor = params[122]  # Cp2残差的待定系数
    ...
    # 将残差项相乘并添加到预测中
    Cp1_residual = X[:, 23].reshape(-1, 1)  # Cp1残差转换为二维列向量
    Cp2_residual = X[:, 24].reshape(-1, 1)  # Cp2残差转换为二维列向量

    y_pred += residual_Cp1_factor * Cp1_residual + residual_Cp2_factor * Cp2_residual

    # 返回残差
    return y_pred.flatten() - y

# ==== 模型拟合 ====
params_init = np.zeros(123)  # 初始化参数，考虑到新增的两个残差系数
...
param_names = (
    ...
    ["beta", "gamma", "delta", "f0", "f1", "gamma_slope", "intercept",
     "residual_Cp1_factor", "residual_Cp2_factor"]
)
print("\n🔧 参数拟合结果:")
...
df_result = pd.DataFrame({
    ...
    # "Cp1_residual": Cp1_residual.flatten(),  # 使用计算得到的残差
    # "Cp2_residual": Cp2_residual.flatten()   # 使用计算得到的残差
})
df_result.to_excel("Hvap_prediction_with_slopeT_and_intercept_19group_with_residuals.xlsx", index=False)
