...
Nk_poly = poly.fit_transform(df.iloc[:, 13:32].apply(pd.to_numeric, errors='coerce'))  # 19个基团
...
results = pd.DataFrame({
    ...
    "Vol_measured": y_total,                     # 真实的体积
    "Vol_predicted": y_pred,                     # 预测的体积
...
# 输出结果并保存
results.to_excel("Vol_prediction_with_slope_and_residuals.xlsx", index=False)
print("✅ 预测结果已保存为 Vol_prediction_with_slope_and_residuals.xlsx")
