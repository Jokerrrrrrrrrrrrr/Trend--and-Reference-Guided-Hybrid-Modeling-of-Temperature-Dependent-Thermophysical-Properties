...
for i in range(len(df)):
    ...
    try:
        ...
        # 计算 A * exp(B)
        A_exp_B = A * np.exp(B)  # 计算 A * exp(B)
        A_exp_B_all.append(A_exp_B)

    except Exception as e:
        print(f"物质 {df.iloc[i, 0]} 拟合失败: {e}")
        A_all.append(np.nan)
        B_all.append(np.nan)
        A_exp_B_all.append(np.nan)
...
for i, row in df.iterrows():
    ...
    A_exp_B = A_exp_B_all[i]  # 使用 A_exp_B_all 列表中的值替代原来的 slope

    for T, vol in zip(temps, vols):
        if np.isnan(T) or np.isnan(vol) or np.isnan(A_exp_B):
            continue
        features = np.concatenate([Nk, [T], [A_exp_B]])  # 使用 A * exp(B) 作为特征
...
results.to_excel("Vol预测结果_加A_exp_B特征_RF.xlsx", index=False)
print("✅ 已保存预测结果为: Vol预测结果_加A_exp_B特征_RF.xlsx")
